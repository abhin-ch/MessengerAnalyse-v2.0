...
dictionary = {'facet_adventurousness_low':"You enjoy familiar routines and prefer not to deviate from them. " ,
...
interest = {'Likely to like romance movies': "Romantic",
...
def big_four(p):
    ...
    [Agreablness, Concisnetous, Openess, Emotional Range]
...
def summary(traits, high=0.75, low=0.45):
    """"
    This function takes in a the sub traits with there coresponding scores
    ...
    for trait in traits:
        ...
        if(traits[trait] > high):
            search_query = trait+"_high"
            response = dictionary[search_query]
            profile = profile + response
        # EXTREMLY LOW
        if(traits[trait] < low):
            search_query = trait+"_low"
            response = dictionary[search_query]
            profile = profile + response
    # False Safe incase the personality is to vibrant 
    try:
        if(len(profile)>650):
            return summary(traits, 0.95,0.10)
    except:
        return ("We Appologize we were'nt able to generate the summary based on your personality at the moment. Please try again later! Thanks :)")        
    return profile
